...
public class CameraController : MonoBehaviour
{
    ...
    private void MoveCamera(){
        if(Input.mousePosition.y >= Screen.height * 0.95f && max_Z > transform.position.z){
            transform.Translate(Vector3.forward * Time.deltaTime * ScrollSpeed, Space.World);
        } else if (Input.mousePosition.y <= Screen.height * 0.05f && min_Z < transform.position.z){
            transform.Translate(-1 * Vector3.forward * Time.deltaTime * ScrollSpeed, Space.World);
        } 
        if (Input.mousePosition.x <= Screen.width * 0.05f && min_X < transform.position.x){
            transform.Translate(Vector3.left * Time.deltaTime * ScrollSpeed, Space.World);
        } else if (Input.mousePosition.x >= Screen.width * 0.95f &&  max_X > transform.position.x){
